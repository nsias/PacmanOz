...
%%%%%%%%% MESSAGES FUNCTIONS %%%%%%%%%%%%%%%%%%%%%
  ...
  fun{PacmanPos ID P State}     % this function updates the position of the pacman with id = ID 
     NewState
     NewTabPos
  in
...
%%%%%% MOVE FUNCTIONS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%Ã¹
  ...
  proc{FindCloser X Y L PacPos PacID}         % find the closer ennemy and place it's id and it's position in the 
     fun{FindCloserAcc X Y L MinDist MinID I} % variables PacPos and PacID
  ...
  fun{Check X Y L}   % this function check if the position pt(x:X y:Y) is a wall or not
    Val in           % she returns pt(X Y)|L if it's not a wall and 'null'|L otherwise
    if X < 1 then 'null'|L
  ...
  fun{Pref X1 Y1 X2 Y2 Mode}    % return a list of the 4 directions ordered by preference for the ghost
     NordDist
     SudDist
     WestDist
     EastDist
  ...
  fun{QuickSort L Mode}        % Sort the List of directions
  ...
  fun{CreateTab Val N}   % creates a new list with N times the elemnts Val
     if N > 0 then Val|{CreateTab Val N-1}
     else nil end
  end

  fun{ModTab Tab N Val}  % modify the Nth elements of the list Tab by Val
  ...
  fun{Reduce L Val}   % reduces the list L by deleting every elements = Val
     case L of H|T then
  ...
  fun{AllNull L}     % returns true if every elements of L is equal to 0
     case L of H|T then
  ...
  fun{Append L1 L2}  % append 2 lists
     case L1 of X|M1 then X|{Append M1 L2}
     [] nil then L2 end
  end

  fun{Replace L Val NewVal}  % replace the first elements of the list L equal to Val by NewVal
     case L of H|T then
